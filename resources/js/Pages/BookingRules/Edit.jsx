...
export default function Edit({ bookingRule, weekdays }) {
    const { data, setData, patch, processing, errors } = useForm({
        label: bookingRule.label ?? '',
        weekday: String(bookingRule.weekday),
        time: bookingRule.time,
        enabled: bookingRule.enabled,
        waitlist_ok: bookingRule.waitlist_ok,
        sort_order: bookingRule.sort_order ?? 0,
    });

    const submit = (e) => {
        e.preventDefault();
        patch(route('booking-rules.update', bookingRule.id));
    };

    return (
        <AuthenticatedLayout header="Időpontok / Szerkesztés">
            <Head title="Időpont szerkesztése" />

            <div className="max-w-lg bg-white rounded-xl shadow-card">
                <div className="px-6 py-4 border-b">
                    <h1 className="text-lg font-semibold text-gray-800">Időpont szerkesztése</h1>
                </div>
